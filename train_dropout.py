            ...
            with torch.no_grad():
                ...
                with tqdm(total=steps_per_epoch_test, leave=False, dynamic_ncols=True) as pbar1:
                    for s, val_batch in enumerate(tqdm(args.loader.test_iter, desc='test')):
                        ...
                        temp = [F.softmax(args.model(feature), dim=1).view(1,feature.size()[0],10) for _ in range(30)]
...
def main():
    ...
    parser.add_argument('--experiment', type=str, default='1206_dropout', help='experiment name')
    ...
    args.model = Dropout(args).cuda(args.gpu_device)
